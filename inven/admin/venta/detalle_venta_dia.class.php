...
if (isset($fecha_dia) && !empty($fecha_dia) && isset($limite)) {

	//Instanciar el Objeto
	$venta = new Consulta();
	//Llamar a la función dia
	$venta->dia($limite, $fecha_dia);
...
class Consulta{
	...
	//Extrae la venta de un día específico establecido por el ussuario
	function dia($limite, $fecha_dia){
		...
		if (mysqli_num_rows($resultado) > 0)
		{
			...
			if ($limite > 0) {	
				...
				$limit = $limite -15;
				echo '<div id="anterior" onclick= "detalle_venta_dia('.$limit.')"><span class="flecha_anterior"></span>Anterior</div>';
			...
			if($limite < $totalReg -15){
				...
				$limit = $limite + 15;
				echo '<div id="siguiente" onclick="detalle_venta_dia('.$limit.')">Siguiente<span class="flecha_siguiente"></span></div>';			
