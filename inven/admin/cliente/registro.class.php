...
function registroValidarNuevo(){
	...
	// Validar cada una de las variables enviadas al servidor
	if (isset($codigo) && !empty($codigo)  && isset($nombre) && !empty($nombre) && isset($apellidos) && !empty($apellidos) &&
		isset($telefono) && !empty($telefono) && isset($mail) && !empty($mail) && isset($direccion) && !empty($direccion) && 
...
class Registro{
	...
	function registroNuevo(){
		...
		$insert = mysql_query("INSERT INTO clientes () VALUES ('$codigo', '$nombre','$apellidos','$telefono','$mail',
	...
	function registroEditar(){
		...
		$update = mysql_query("UPDATE  clientes SET nombre='".$nombre."',apellidos='".$apellidos."',telefono='".$telefono."',
		                      mail='".$mail."',direccion='".$direccion."',cedula='".$cedula."',sexo='".$sexo."',
		                      fecha_ingreso='".$fecha_ingreso."' WHERE id_cliente= '".$codigo."' ", $cn);
