...
class Consulta{
	...
	function consultarCodigo(){
		...
		$query = "SELECT * FROM tipo_usuarios WHERE id_tipo_usuario ='".$id."'";
		...
		if (mysqli_num_rows($query_db) > 0){
			...
			while ($fila = mysqli_fetch_array($query_db)) {
				...
				echo '<tr>'; 
			    echo '<td><input type="text" name="dcodigo" id="dcodigo" value="'.$fila['id_tipo_usuario'].'" readonly /></td>';			   
	...
	function consultarDescripcion(){
		...
		// Consulta a la base de datos
		$query = "SELECT * FROM tipo_usuarios WHERE descripcion LIKE '".$descripcion."%'";
		$query_db = mysqli_query($cn,$query);
	...
	function consultarTodo(){
		...
		// Primera consulta a la tabla Tipo Usuario
		$query = "SELECT * FROM tipo_usuarios limit $limite, 18";
		$query_db = mysqli_query($cn,$query);

		// Saber cuento registros tiene la tabla Pro
		$reg = "SELECT * FROM tipo_usuarios";
