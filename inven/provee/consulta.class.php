...
class Consulta{
	...
	function consultarCodigo(){
		...
		if (mysqli_num_rows($query_db) > 0){
			...
			echo '<table id="tablaProvee">';					
			echo '<tr>';
			echo '<th><input type="text" name="codigo" id="codigo" value="Código" readonly /></th>';			
			echo '<th><input type="text" name="id_empresa" id="id_empresa" value="Empresa" readonly /></th>';			
			...
			echo '<th><input type="text" name="telefono" id="telefono" value="Telefono" readonly /></th>';
			...
			while ($fila = mysqli_fetch_array($query_db)) {
				...
				echo '<tr>'; 
			    echo '<td><input type="text" name="dcodigo" id="dcodigo" value="'.$fila['id_proveedor'].'" readonly /></td>';			    
	...
	function consultarTodo(){
		...
		$query = "SELECT * FROM proveedores limit $limite, 18";
		$query_db = mysqli_query($cn,$query);

		// Saber cuento registros tiene la tabla Proveedores
		$reg = "SELECT * FROM proveedores";
