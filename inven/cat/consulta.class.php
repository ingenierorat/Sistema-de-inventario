...
class Consulta{
	...
	function consultarCodigo(){
		...
		if (mysqli_num_rows($query_db) > 0){
			...
			while ($fila = mysqli_fetch_array($query_db)) {
				...
				echo '<tr>'; 
			    echo '<td><input type="text" name="dcodigo" id="dcodigo" value="'.$fila['id_categoria'].'" readonly /></td>';			   
	...
	function consultarNombre(){
		...
		// Consulta a la base de datos
		$query = "SELECT * FROM categoria WHERE descripcion LIKE '".$descripcion."%'";
		$query_db = mysqli_query($cn,$query);
	...
	function consultarTodo(){
		...
		$query = "SELECT * FROM categoria limit $limite, 18";
		$query_db = mysqli_query($cn,$query);

		// Saber cuento registros tiene la tabla Pro
		$reg = "SELECT * FROM categoria";
